...
async def create_new_budget_group(
    new_budget_group: BudgetGroupCreate, db: AsyncDb
) -> BudgetGroupOrm:
...
# Anyway, the async engine would have resulted in runtime errors when those
# serialization queries run, so this option solves two problems: query efficiency, and
# avoid runtime error for async.
...
@router.post("/groups/{budget_group_id}/update", response_model=BudgetGroupResponse)
async def update_budget_group(
    budget_group_id: int, new_fields: BudgetGroupUpdate, db: AsyncDb
) -> BudgetGroupOrm:
    group_updated_orm = await db.get(
        entity=BudgetGroupOrm,
        ident=budget_group_id,
    ...
    for item_id in new_item_ids:
        ...
        if item_orm is None:
            raise HTTPException(
                status_code=422, detail="Bad request: budget item doesnt exist."
            )
...
async def delete_budget_group(budget_group_id: int, db: AsyncDb) -> None:
...
async def create_new_budget_item(
    new_budget_item: BudgetItemCreate, db: AsyncDb
) -> BudgetItemOrm:
    ...
    # Runs the query, makes the transaction pending, sets the id to the
    # postgres-generated id
    await db.flush()
    await db.commit()
    return new_item_orm
...
async def update_budget_item(
    budget_item_id: int, updated_budget_item: BudgetItemUpdate, db: AsyncDb
) -> BudgetItemOrm:
...
async def delete_budget_item(budget_item_id: int, db: AsyncDb) -> None:
